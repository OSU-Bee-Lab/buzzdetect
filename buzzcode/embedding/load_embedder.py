...
def load_embedder(embedder_name: str, framehop_prop: float, load_model: bool):
    """
    ...
    """
    embedder_path = Path(cfg.DIR_EMBEDDERS) / embedder_name

    if not embedder_path.exists():
        raise ValueError(f"Embedder '{embedder_name}' not found in {cfg.DIR_EMBEDDERS}")

    # Import the embedder module
    spec = importlib.util.spec_from_file_location(
        f"{embedder_name}_embedder",
    ...
    if embedder_class is None:
        raise ValueError(f"No BaseEmbedder subclass found in {embedder_name}/embedder.py")
