...
# modelname = 'test'; metadata_name="metadata_strict"; weights_name=None; epochs_in=3; embeddername='yamnet'
def generate_model(modelname, embeddername='yamnet', metadata_name="metadata_strict", epochs_in=100):
    input_size = 1024  # not sure how to handle this; input shape will be fixed for any one model but is still in flux
    ...
    dir_cache = os.path.join(dir_training, 'input_cache')
    ...
    def load_embed(path_inputs, target, fold):
        embed_array = tf.numpy_function(
            func=lambda y: np.array(np.load(y.decode("utf-8"), allow_pickle=True))[0:1024],
            inp=[path_inputs],
            Tout=tf.float64,
        )
