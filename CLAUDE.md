...
npm run build:engine       # freeze the engine into engine-payload/ (see Releases)
...
- **Rust** (`src-tauri/src/lib.rs`, `mod tests`). Only the pure edges:
  ...
  `cargo test` needs nothing built — the engine is a bundled resource now, not
  a `tauri.conf.json` `externalBin`, so tauri's build script no longer insists
  on a sidecar file being present.
...
Outside this repo, buzzdetect-training's `04_deploy/export_onnx.py` re-exports a
...
- **The frozen engine is a onedir, not a onefile, and that is a startup-latency decision.** A onefile binary re-extracts its ~200MB of libraries to a fresh temp directory on every launch and imports them cold; measured on the packaged app that was ~30s from Launch Analysis to the first chunk, *every* run. onedir ships the libraries unpacked in `engine-payload/engine-bin/`, so a launch whose files the OS still has page-cached is ~0.5s instead (measured warm). How often that cache survives between real launches wasn't measured — a cold launch (after a reboot, or once the OS has evicted 200MB of dylibs under memory pressure) is still ~30s, so the first launch after install always is. That wait is covered in the UI by the `starting` stage ("Loading engine components…") and the animated header ellipsis; the stage is the engine importing numpy/pandas/the audio stack, not onnxruntime (which loads later, per worker, under `loading`). The cost of onedir is that the frozen engine is a directory, so it rides in the resource payload instead of being a `tauri.conf.json` `externalBin` — `resolve_engine` spawns `engine-payload/engine-bin/buzzdetect-engine`, and in a checkout `npx tauri dev` will use it too if the payload has been built (delete `src-tauri/engine-payload/engine-bin/` to force the `engine/.venv` path). The onedir is ~170MB unpacked vs the ~60MB compressed onefile, which eats into the Windows CUDA portable zip's headroom under GitHub's 2GiB asset limit.

- **Subprocess boundary, not a library call.** `src-tauri/src/lib.rs::start_analysis` spawns the engine as a child process. `resolve_engine` picks between two shapes: the frozen PyInstaller engine at `engine-payload/engine-bin/` inside the app's resources (shipped builds), or `engine/.venv` + `buzzdetect_cli.py` (a checkout). Either way the child's working directory must contain `models/` and `src/stream/drivers/`, because `engine/src/config.py` addresses both by relative path — that's the whole reason `scripts/build-engine.mjs` assembles `src-tauri/engine-payload/` rather than freezing them into the binary. Only one analysis runs at a time (`AnalysisState` mutex). Stopping is a request, not a kill: `cancel_analysis` writes `STOP` on the child's stdin (which is why that pipe is kept open after the manifest prompt's `y`), the engine takes that as the coordinator's early-exit path, and the logs keep flowing while the workers wind down and report themselves out. Signals are only the escalation, and only once the engine has gone quiet — `engine/src/pipeline/interrupt.py` is the engine end of that, where a signal, or a second stop request, is the impatient path. The engine's own stdout/stderr lines prefixed `BDPROGRESS ` (`PROGRESS_MARKER`) are JSON and get re-emitted as Tauri's `engine-progress` event; everything else becomes `engine-log`. `emit_progress()` in `engine/src/pipeline/progress_json.py` is the single source of truth for that wire format — if you add a new progress event kind, update both it and the frontend's `run` store (`src/lib/progress.svelte.ts`) together.
...
- **A model is one ONNX graph, waveform in, predictions out.** There is no embedder plugin, no NumPy front end and no TensorFlow anywhere in the engine: `models/<name>/model.onnx` contains the log-mel front end, the embedding trunk and the classifier head, fused into a single graph by buzzdetect-training's `04_deploy/export_onnx.py`. A model directory is that file, an optional `model.fp16.onnx`, and a `config_model.json` — the class list *and* the framing parameters (`samplerate`, `framelength_s`, `digits_time`, `digits_results`, `samples_hop`, `samples_min`); `engine/src/inference/models.py`'s `REQUIRED_CONFIG_KEYS` is the authoritative list and `_validate_config` rejects a dir missing any. Nothing in a model directory is executable — a model is data (this is why: `import_model` in `src-tauri/src/lib.rs` copies collaborator-supplied folders in, and code there would run with the app's privileges). The `.onnx` files are build artifacts: retrain a model or change the front end and you re-export, you don't derive anything at runtime. `shipped-models.txt` is the list of names that get bundled; everything else in `engine/models/` stays local.
- **Models resolve by name across several roots.** `cfg.model_roots()` is the bundled `models/` dir (found via the engine's cwd) followed by every absolute path in `BUZZDETECT_MODELS_PATH` (os.pathsep-joined). `cfg.resolve_model_dir(name)` walks them in order — bundled shadows imported on a name collision. The desktop app sets that env var to its per-user store, `app_local_data_dir()/models/`, which is where `import_model`/`remove_model` write; it's outside the app bundle on purpose (writing into a signed `.app` or Program Files needs privileges and breaks the signature). `import_model` takes either a folder or a `.zip` of one (the `zip` crate; extracted to a temp dir, `find_model_root` handles a zip that wraps its files in a subfolder) and validates it against the same required keys before copying — a model dir there is inert data, so nothing runs on import. `list_models` (Rust) scans the same roots and tags each result `removable` (i.e. user-imported). If you add a model root, add it to both `cfg.model_roots()` and `model_roots()` in `lib.rs`.
...
- **A model directory can also carry things for people, which the engine ignores.** `config_model.json` may hold a hand-written `description` (shown under the model picker) and `thresholds` (`{class: number}`) plus `threshold_stats`, both written by buzzdetect-training's `03_train/thresholds.py` and carried over by `export_onnx.py` along with `README.md`. The Info button opens the `model-info` window (`src/routes/model-info/`, created by `open_model_info` in `lib.rs`; its label is in `capabilities/default.json`), which renders the README as sanitized markdown under a thresholds table. `thresholds` and the model's `framelength_s` are also recorded in `buzzdetect_manifest.json` as informational keys (`KEYS_INFO` in `pipeline/manifest.py`) — never compared, and back-filled into an older manifest; SeeNote and buzzr read `thresholds` from there.
- `engine/src/gui/` is the legacy CustomTkinter GUI, superseded here by the Tauri frontend; don't extend it. Same for `engine/audio_in/` and `engine/buzzdetect_gui.py` — they are excluded at bundle time rather than deleted. That was originally to avoid conflicting on every subtree pull; with the repos merged, deleting them is merely a decision nobody has made, so leave them alone unless asked. The Sphinx docs that used to live at `engine/docs/` are now `docs/` at the repo root, alongside `docs/linux-gpu.md`; `.readthedocs.yaml` moved to the repo root with them, so readthedocs.org's project settings need their "Path for .readthedocs.yaml" reset to the default (or repointed) to match.
